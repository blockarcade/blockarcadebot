  ...
  iostPOSTRequest('/getContractStorage', {"id":"Contract6sCJp6jz2cpUKVpV6utA1qP5BxFpHNYCYxC6VAMpkCq5","key":"leaderboardReward","by_longest_chain":true}, (err, response) => {
    ...
    iostPOSTRequest('/getContractStorage', {"id":"Contract6sCJp6jz2cpUKVpV6utA1qP5BxFpHNYCYxC6VAMpkCq5","key":"issuedTIXUsers","by_longest_chain":true}, (err, response) => {
      ...
      iostPOSTRequest('/getBatchContractStorage', {"id": "Contract6sCJp6jz2cpUKVpV6utA1qP5BxFpHNYCYxC6VAMpkCq5","key_fields": fields, "by_longest_chain": true}, (err, response) => {
        ...
        const newScores = scores.filter(a => a.user !== 'octalmage').filter(a => a.user !== '0ctane').slice(0, 10)
    ...
    iostRequest('/getTokenBalance/ContractEnn4aBKJKwqQCsQiqFYovWWqm6vnA6xV1tT1YH5jKKpt/itrx/true', (err, response) => {
      ...
      iostRequest('/getTokenBalance/ContractEnn4aBKJKwqQCsQiqFYovWWqm6vnA6xV1tT1YH5jKKpt/tix/true', (err, response) => {
        ...
        iostRequest('/getTokenBalance/ContractEnn4aBKJKwqQCsQiqFYovWWqm6vnA6xV1tT1YH5jKKpt/iplay/true', (err, response) => {
          ...
          postGifToTelegram(cashGif, `*Major jackpot is up to ${numberWithCommas((iostBalance / 10).toFixed(2))} $IOST, ${numberWithCommas((itrxBalance/ 10).toFixed(2))} $ITRX, ${numberWithCommas((tixBalance / 10).toFixed(2))} $TIX, ${numberWithCommas((iplayBalance / 10).toFixed(2))} $IPLAY and ${numberWithCommas((lolBalance / 10).toFixed(2))} $IPLAY!*\n\nWho's going to win it?\n\nPlay now at: https://blockarca.de`);
        });
      });
      });
    });
